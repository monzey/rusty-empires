...
use super::components::{Building, MapPosition, Tile, Unit, UnitKind};
...
use crate::{BuildingKind, Camp, GridPosition, NaturalResource, UnitId};
...
fn spawn_villager(commands: &mut Commands, id: UnitId, camp: Camp, grid_position: GridPosition) {
    let kind = UnitKind::Villager;

    commands.spawn((
        SpriteBundle {
            sprite: Sprite {
                color: unit_color(camp, kind, false, false),
                custom_size: Some(Vec2::splat(TILE_SIZE * 0.62)),
                ..default()
            },
